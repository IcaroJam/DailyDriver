<script>
	...
	function swirlOut() {
		return {
			duration: 200,
			css: (t) => `transform: rotate(${t * -180}deg); opacity: ${t}`
		}
	}
</script>
...
{#if oldVal != newVal}
	<button class="restoreIcon" title="Restore!" on:click|preventDefault={restore} in:fade={{ duration: 200 }} out:swirlOut>
